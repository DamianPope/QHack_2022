...
def compare_circuits(angles):
    ...
    # QHACK #
    return abs(circuitOne(angles) - circuitTwo(angles))

#define a device and quantum functions/circuits here
#define a backend “device” that executes the circuits
#default.qubit = default simulator
dev1 = qml.device("default.qubit",wires=1)

@qml.qnode(dev1)
def circuitOne(params):
    qml.RX(params[0], wires=0)
    qml.RY(params[1], wires=0)
    return qml.expval(qml.PauliX(0))

@qml.qnode(dev1)
def circuitTwo(params):
    qml.RY(params[1], wires=0)
    qml.RX(params[0], wires=0)
    return qml.expval(qml.PauliX(0))
    # QHACK #
